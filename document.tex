...
From  diagrams: essentially we want to make sure that our Lyapunov function is strictly increasing and has no minima or maxima. If $\dot{V}(x)$ is ND $\in \mathcal{R}$, then $V(x) \rightarrow 0$. This also implies that $x\rightarrow 0$ as the PD function $V(x)$ only has a zero point at the origin. This is GAS.
...
If V is PD on D (containing $\bar{x}$) and $\dot{V}$ is PD on D $\Rightarrow \bar{x}$ is unstable ISL. The book talks about Chetaev's theorem (4.3) is stronger as it has a weaker condition on $\dot{V}$.
...
\subsection*{Why are we doing this crap?} % (fold)
...
% subsection why_are_we_doing_this_crap_ (end)
...
\section*{April 22nd: Finding the Describing Function}
Let us recall that we would insert a sinusoidal function into a nonlinearity. We see the spectrum of the output, and can selectively generate an approximate function that describes the dominant frequency.

How do we find this exactly? We must also remember there exists a transient. This decays, leaving the steady state, given the function is stable.

We can describe the steady state output as such

\begin{equation}
	y_{ss} = \hat{C}_0  + \sum_{k = -\infty}^{\infty} \hat{C}_k e^{j (\omega k) t}
\end{equation}

Where $\omega$ is the fundamental while $\omega k$ is the harmonic frequency. The spectral distribution amplitude is denoted with $\hat{C}_k$. It is defined by

\begin{equation}
	\hat{C}_k = \frac{1}{T} \int_0^T y_{ss}(t)e^{-j(\omega k)t} dt
\end{equation}

Now we must substitute $y_{ss}(t)$ can carry out the integration. We want to find the ratio of the input and output spectral amplitudes. We can describe the input as such:

\begin{align}
	u(t) &= a\sin(\omega t) = \frac{a (e^{j \omega t}- e^{-j \omega t})}{2j} \\
	&= \hat{a}_0 + \sum_{k = - \infty}^\infty \hat{a}_k e^{j (\omega k) t}
\end{align}

In the case where $\hat{a}_0 = 0$ we see that $\hat{a}_1 = \frac{a}{2j}$. In our example suppose the nonlinearity is $\psi(u) = y_{ss}$. Lets take the case of a saturation nonlinearity (drawn on paper). The saturation points on the output vs input plot are at $+\delta, \ -\delta$. For $\lvert u \rvert < \delta \rightarrow y = ku$ and we are ok, and the output stays sinusoidal. When $\lvert u \rvert > \delta$ so we get clipping on the output waveform. The amplitude of the first harmonic can be approximated by a slightly smaller sinusoidal function. 

We can state that $\lvert \hat{C}_1 \rvert < k$. The first harmonic will not change as the input grows. Our describing function is $\Psi = \frac{\hat{C}_1}{\hat{a}_1}$ which is the ratio of amplitudes as a gain. Linear plot of the relationship on paper. Typically these describing functions are precomputed and are in tables in the older texts.

The ratio:
\begin{align}
	\Psi &= \frac{\hat{C}_1}{\hat{a}_1} = \frac{2j}{a} \frac{\omega}{2\pi}\int_0^T \psi(a\sin(\omega t)) e^{-j(k\omega)t} dt \\ 
	&= \frac{2j}{a} \frac{\omega}{2\pi}\int_0^T \psi(a\sin(\omega t)) (cos(\omega t) - j\sin(\omega t)) dt
\end{align}

The cosine portion averages to zero of the integral but the $\sin^2$ term does not leaving us with the following:

\begin{align}
	\Psi &= \frac{-2j^2}{a} \frac{\omega}{2\pi}\int_0^T \psi(a\sin(\omega t))\sin(\omega t) dt \\
	&= \frac{1}{a \pi} \int_0^{2\pi} \psi(a\sin(\theta))\sin(\theta) d\theta \\
	& = \frac{2}{a \pi} \int_0^{\pi} \psi(a\sin(\theta))\sin(\theta) d\theta
\end{align}

We see this is not dependent upon time.
 
NOTE: read book for more information on $\psi$. Lowercase is the nonlinearity, uppercase is  the describing function.





\section*{April 25th, 2019: Feedback Linearization}
Robot manipulator example, with joint deflections $q \in \mathbb{R}^n$, where $n$ is usually 6 for three positional and attitude DOF. We can get them into this form:

\begin{equation}
	H(q)\ddot{q} +  S(q,\dot{q})\dot{q} + g(q) = \tau
\end{equation}

All of the elements in this equation are torques, with $\tau$ representing joint torques. We see immediately that $H(q)$ is the inertial torque, and that $S(q,\dot{q})$ is a damping kind of torque and other stuff. This second portion includes Coriolis gyroscope, damping, and centripetal type torques. 

How do we generate a control torque which linearizes the dynamics? First we must find a way to linearize the dynamics and then make sure it tracks our control references.

Suppose $\tau = H(q)v + S(q,\dot{q})\dot{q} + g(q)$. Plugging in we have that, assuming that $H(q)^{-1}$ exists $\forall \ q$:

\begin{equation}
	H(q)(\ddot{q} - v) = 0
\end{equation}

In this case we have that $\ddot{q} = v$ in the nontrivial case. We can see the system as v being double integrated to get $q$. This double integrator is a linear, decoupled, second-order system. Given that it is decoupled, it is a stackup of individual differential equations. There are no zeros, as it looks like $\frac{1}{s^2}$. So now let us do some control design to make this thing track a reference $q_r$.

This could be $v = K_p (q_r - q) - K_d\dot{q}$. However, the dynamics are very complicated and it is very hard to implement. Most people would not implement this, as the cancellation terms are very difficult to calculate with the complicated dynamics. In most scenarios the middle term has small Coriolis and gyroscopic forces. Additionally, with large gear ratios, the gravity backdrive ends up being very small. The inertia terms dominate by far. The $H$ matrix ends up being almost diagonal. Most people ended up only implementing a PD controller at each joint with high gains. 

If we crank out the linearization it ends up looking roughly like:

\begin{equation}
	M\ddot{q} + B\dot{q} + Kq = \tau
\end{equation}

For matrices M, B, and K. The decoupled case ends up being approximately:

\begin{align}
	& m_i \ddot{q}_i + b_i \dot{q}_i + k_i q = \tau_i \\
	& m_i \ddot{q}_i  = \tau_i -b_i \dot{q}_i - k_i q
\end{align}

With the transfer function being:

\begin{equation}
	\frac{q_i(s)}{\tau_i(s)} = \frac{1}{m_i s^2 + b_i s + k_i}
\end{equation}

The block diagram is written on paper. We see that $q_i$ is related to $\tau_i$ through a double integrator process. He says that $q_i$ is "two integrators away" from $\tau_i$, independent of the feedback.

Now, as shown on paper, the feedback linearization function effectively becomes the new dynamics function. We add another feedback loop to add more zeros to the system. We use our own feedback to cancel the internal feedback! Interesting to note what is written on paper.

Now let us try to add zeros to the reduced chain of integrators. We have the standard $y(s) = H(s)u(s)=  \frac{N(s)}{D(s)}u(s)$ with denominator of order m and denominator of order n. It must be proper to be realizable, therefore $m \leq n$. We also have the idea of relative degree where $p = n-m$. Proper for $p=0$ and strictly proper for $p>0$. Obviously the relative degree of the chain of integrators was 2.

Ideally we add feedback such that we reduce it to chain of integrators.

Now we describe the system with $D(s)y(s) = N(s)u(s)$. So if we could divide N into D, we get a quotient and a remainder. Therefore $D(s) = N(s)Q(s) + R(s)$ with $deg(Q(s)) = p$ and with $deg(R(s)) < deg(N)$.

We now have that $(NQ + R)y = Nu $ which is manipulated to 
...
This block diagram, on paper, is now our $H(s)$ which is an integrator with internal feedback. We can cancel this internal feedback. It becomes a reduced order system as we have lost some of the poles. If we look at the new transfer function we have that

\begin{align}
	\frac{y}{r} &= \frac{\frac{N}{D}}{1-(\frac{R}{N}\frac{N}{D})} \\ 
	&=\frac{N}{D-R} \\
	&= \frac{N}{(NQ+R) -R} \\
	& = \frac{1}{Q}
\end{align}

However, it is hard to cancel N exactly if you do not have the perfect model. So if there is an unstable zero, it could not cancel perfectly. We must show that the $\mathbb{Re}(\lambda_i(N(s))) < 0$. Another way to say this is that $H(s)$ is minimum phase (zeros are strictly left hand plane) or "zero dynamics of H are stable."





\section*{April 30th, 2019: More on Feedback Linearization}

Recall that we were left with a chain of integrators with our new defined input as $z$. Our system is described by, using the polynomial division version of the block diagram:

\begin{equation}
	e(t) = q_p \frac{d^p y(t)}{dt^p} + ... + q_0 y(t)
\end{equation}

We would like to make a state space model (book usees $\xi$). We have that $\xi_1 = y$, $\xi_2 = \dot{\xi}_1$, and they continue to successive derivatives such that $\xi_p = \dot{\xi}_{p-1} = \frac{d^{p-1} y(t)}{dt^{p-1}}$ and therefore $\dot{\xi}_p = \frac{d^p y(t)}{dt^p} = \frac{1}{q_p}e(t) - \frac{q_{p-1}}{q_p}\xi_p - ... - \frac{q_0}{q_p}\xi_{p-1}$.

So we can describe it as such: $\dot{\xi} = A\xi + Be$ with output equation $y = C\xi$. We see that, with our definition, A is a companion matrix form which has mostly entries of 1 that are off diagonal. The bottom row is all of the constants $q$ that we described in the previous paragraph. If, with feedback, we cancel all of the elements on the bottom row and make this a chain of integrators, then all eigenvalues are at the origin: $A_c + B_c \lambda^T$ for $\lambda^T = [\frac{-q_o}{q_p} \ ... \ \frac{-q_{p-1}}{q_p}]$.

We can then describe a minimal realization of the bottom portion of the block diagram $\frac{R}{N}$.

\begin{align}
	\dot{\eta} = A_0 \eta + B_0 y \\
	w = C_0 y
\end{align}

This is the state space representation of this lower block system. We can stack this up like the following (Normal form):
...
\begin{equation}
	...
	\end{bmatrix} = 
	\begin{bmatrix}
	A_c + B_c \lambda^T & \frac{B_cC_0}{q_p}\\
	B_0C_c & A_0
	\end{bmatrix}
	\begin{bmatrix}
	\xi \\
...
\begin{equation}
	\dot{\xi} = (A_c + B_c \lambda^T) \xi + \frac{B_cC_0}{q_0} \eta + B_c\frac{1}{q_p}u
\end{equation}

We can choose that 

\begin{equation}
	u = q_p(-\lambda^T \xi + \frac{1}{q_p}C_0\eta +v)
\end{equation}
...
\begin{align}
	\dot{\xi} &= A_c \xi + B_c v \\
	y &= C\xi \\
	\dot{\eta} &= B_0C_c\eta + A_0\eta 
\end{align}

Which is a chain of p integrators. This feedback results in a system that appears as:
...
Which is upper triangular and there is a one way dependence. This matrix is companion form and this is a chain of integrators. Ideally we would like to take a nonlinear system and put it into normal form. If we can, then we can do state variable feedback and cancel elements to make a chain of integrators to which we could add linear control design.

So what does it take to get a nonlinear system into a form like this? Suppose, as a special case, we have the system in the form $\dot{x} = f(x) + g(x)u$ called the separable form. Now usually it is like $\dot{x} = f(x,u)$, so this is special. Of course we just say that $y = h(x)$. We assume that $f(x), g(x), h(x)$ are smooth: smooth enough to take the number of derivatives required for the theory. We perform a change of variables, as a general transformation:
...
This needs to be a smooth homeomorphism (diffeomorphism): an invertible mapping that is smooth. Basically we aim to make the $\eta$ states as unobservable on the output. Therefore we need that

\begin{equation}
	\dot{\eta} = \frac{d}{dt} \phi(x) = \frac{\partial \phi}{\partial x} \dot{x} = \frac{\partial \phi}{\partial x}(f(x) + g(x)u)
\end{equation}

We need to choose $\phi(x)$ such that the projection: $\frac{\partial \phi}{\partial x} \cdot g(x) = \bar{0}$. This is not necessarily easy to find this function.

Now we can construct the $\xi$ portion:
...
\begin{equation}
	...
	\end{bmatrix} = \begin{bmatrix}
		...
		\phi_{n-p}(x) \\
		y = h(x) \\
		\dot{\xi}_1 = \frac{d}{dt}(h(x)) = \frac{\partial h}{\partial x}(f(x) + g(x)u) \\
		\dot{\xi}_2 = \frac{d}{dt}(\frac{\partial h}{\partial x}f(x))  = \frac{\partial}{\partial x}(\frac{\partial h}{\partial x}f(x)) \\
		\vdots \\
		\dot{\xi}_p = -\delta(x) + \gamma(x)u
	\end{bmatrix}
\end{equation}

To get $u$ to appear here, the derivative must exceed the relative degree of the system. Therefore the dimensionality of the bottom half of this vector is $p$. Notice how I drop the input multiplication in subsequent steps. The input only shows up on the last step. The bottom row is kind of like the companion form chain of integrators. Then the idea is to use $u$ to get rid of the $\delta$ term. 

We can write:

\begin{equation}
	\dot{\xi}_p = -\delta(x) + \gamma(x)u = \gamma(x) \Bigg(u - \frac{\delta(x)}{\gamma (x)} \Bigg) = \gamma(x) (u - \alpha{x}) 
\end{equation}

We choose that $u = \alpha(x) + \frac{1}{\gamma(x)}$ such that $\dot{\xi}_p = v$. This is a linear chain of integrators. THM 13.1 in the book shows why this is true.

By THM 13.1, such a $T(x)$ always exists with the assumptions of smoothness on some domain D (also must be that $\gamma(x)$ does not go singular) about an x of interest.

Also, he missed the definition of normal form:
\begin{align}
	\dot{\eta} &= f(\eta,\xi) \\
	\dot{\xi} &= A_x\xi + B_c\gamma(x)(u-\alpha(x))\\
	y &= C_0\xi 
\end{align}

And now we see that $\eta$ is internal and do not appear in the input output map. 

Bad things will happen if some of your internal states are blowing up - basically you must know that your internal states are stable. This was an initial assumption. So we must have that $\dot{\eta} = f(\eta,0)$ is asymptotically stable or minimum phase ``zero dynamics are stable."




\section*{May 2nd: Adaptive Control and Exam Questions}
ALL ON PAPER PLEASE SEE THAT


\clearpage
\section*{EXAM NOTES FROM BOOK AND OTHER SOURCES}
All of the small gain and passivity approaches are in the input-output stability category. We just relate the output of the model directly to the input. In this section we define $\mathcal{L}$ stability. We shall calculate the $\mathcal{L}_2$ gain for LTI systems and then study a version of the small-gain theorem.
